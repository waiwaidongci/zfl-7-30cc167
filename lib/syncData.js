...
import { addNote, moveAnimal, getAnimal, getAnimalRoomId } from "./animalData.js";
...
export const CONFLICT_QUEUE_STATUS = {
  PENDING: "pending",
  RESOLVED_SERVER_WINS: "resolved_server_wins",
...
export function ensureSyncCollections(db) {
  ...
  if (!db.conflictQueue) db.conflictQueue = [];
}
...
function resolveRoomIdForOperation(db, operation) {
  const opType = operation.operationType;
  const payload = operation.payload || {};
  ...
  switch (opType) {
    ...
    case SYNC_OPERATION_TYPES.ANIMAL_MOVE:
      if (payload.animalId) {
        return getAnimalRoomId(db, payload.animalId);
      }
      break;
    case SYNC_OPERATION_TYPES.FEEDING_RECORD:
      if (payload.targetType === "animal" && payload.targetId) {
        return getAnimalRoomId(db, payload.targetId);
      } else if (payload.targetType === "cage" && payload.targetId) {
        const cage = getCage(db, payload.targetId);
...
export function addToConflictQueue(db, operation, conflictInfo, options = {}) {
  ensureSyncCollections(db);

  const roomId = resolveRoomIdForOperation(db, operation);
  const animalId = resolveAnimalId(operation);
  const cageId = resolveCageId(operation);
  ...
  const conflictItem = {
    ...
    cageId,
    date: resolveOperationDate(operation),
    status: CONFLICT_QUEUE_STATUS.PENDING,
    incomingPayload: operation.payload || {},
    existingOperationId: conflictInfo?.existingOperation?.operationId || null,
    existingKeeper: conflictInfo?.conflictDetails?.existingKeeper || null,
    existingSubmittedAt: conflictInfo?.conflictDetails?.existingSubmittedAt || null,
    conflictDetails: conflictInfo?.conflictDetails || null,
    originalSyncStatus: options.originalSyncStatus || null,
    originalMergeStrategy: options.originalMergeStrategy || null,
  ...
  db.conflictQueue.push(conflictItem);
  return conflictItem;
}

export function getConflictById(db, conflictId) {
  ensureSyncCollections(db);
  return db.conflictQueue.find((c) => c.id === conflictId) || null;
}

export function listConflicts(db, filters = {}) {
  ensureSyncCollections(db);
  let items = [...db.conflictQueue];

  if (filters.status) {
    items = items.filter((c) => c.status === filters.status);
  }
  if (filters.keeper) {
    items = items.filter((c) => c.keeper === filters.keeper);
  }
  if (filters.roomId) {
    items = items.filter((c) => c.roomId === filters.roomId);
  }
  if (filters.operationType) {
    items = items.filter((c) => c.operationType === filters.operationType);
  }
  if (filters.fromDate) {
    items = items.filter((c) => c.createdAt >= filters.fromDate);
  }
  if (filters.toDate) {
    items = items.filter((c) => c.createdAt <= filters.toDate);
  }
  if (filters.animalId) {
    items = items.filter((c) => c.animalId === filters.animalId);
  }
  if (filters.cageId) {
    items = items.filter((c) => c.cageId === filters.cageId);
  }

  return items.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export async function resolveConflict(db, conflictId, strategy, resolver = null, note = null) {
  ensureSyncCollections(db);

  const conflict = getConflictById(db, conflictId);
  if (!conflict) {
    return { ok: false, error: "conflict_not_found", message: `冲突记录 ${conflictId} 不存在` };
  }

  if (conflict.status !== CONFLICT_QUEUE_STATUS.PENDING) {
    return { ok: false, error: "conflict_already_resolved", message: `冲突已处理，当前状态: ${conflict.status}` };
  }

  if (strategy !== CONFLICT_STRATEGY.SERVER_WINS && strategy !== CONFLICT_STRATEGY.CLIENT_WINS) {
    return { ok: false, error: "invalid_strategy", message: `不支持的解决策略: ${strategy}，仅支持 server_wins 或 client_wins` };
  }

  if (strategy === CONFLICT_STRATEGY.SERVER_WINS) {
    conflict.status = CONFLICT_QUEUE_STATUS.RESOLVED_SERVER_WINS;
    conflict.resolutionStrategy = strategy;
    conflict.resolvedAt = new Date().toISOString();
    conflict.resolvedBy = resolver?.name || null;
    conflict.resolutionNote = note;
    return { ok: true, data: { conflict: { ...conflict }, applied: false, note: "采用服务端数据，无需修改业务记录" } };
  }

  const existingOp = conflict.existingOperationId
    ? getSyncOperationById(db, conflict.existingOperationId)
    : null;

  if (!existingOp) {
    return { ok: false, error: "existing_operation_not_found", message: "无法定位原始服务端操作记录，无法执行客户端优先合并" };
  }

  const effectivePayload = { ...conflict.incomingPayload };

  const mergeResult = await mergeExistingRecord(db, existingOp, effectivePayload, strategy, { operator: resolver });

  if (!mergeResult.ok) {
    return { ok: false, error: mergeResult.error, message: mergeResult.message };
  }

  conflict.status = CONFLICT_QUEUE_STATUS.RESOLVED_CLIENT_WINS;
  conflict.resolutionStrategy = strategy;
  conflict.resolvedAt = new Date().toISOString();
  conflict.resolvedBy = resolver?.name || null;
  conflict.resolutionNote = note;
  conflict.resolvedData = mergeResult.data;

  return { ok: true, data: { conflict: { ...conflict }, applied: true, result: mergeResult.data } };
}

export function dismissConflict(db, conflictId, resolver = null, note = null) {
  ensureSyncCollections(db);
  ...
  conflict.status = CONFLICT_QUEUE_STATUS.DISMISSED;
  conflict.resolvedAt = new Date().toISOString();
  conflict.resolvedBy = resolver?.name || null;
  conflict.resolutionNote = note;

  return { ok: true, data: { conflict: { ...conflict } } };
}
