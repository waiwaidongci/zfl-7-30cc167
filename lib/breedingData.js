...
export function calculateWeaningForecast(db, options = {}) {
  ...
  const unweanedLitters = (db.breedingLitters || []).filter((l) => {
    if (l.status === LITTER_STATUS.WEANED) return false;
    if (!l.birthDate) return false;

    const expectedWeanDate = addDays(l.birthDate, weaningDays);
    if (!expectedWeanDate) return false;

    if (expectedWeanDate > endDateStr) return false;
    if (expectedWeanDate < today) {
      expectedWeanDate = today;
    }

    l._expectedWeanDate = expectedWeanDate;
    return true;
  });

  if (roomId) {
    for (const litter of unweanedLitters) {
      const enriched = enrichLitter(db, litter);
      if (enriched.roomId !== roomId) {
        const idx = unweanedLitters.indexOf(litter);
        if (idx > -1) unweanedLitters.splice(idx, 1);
      }
    }
  }
  ...
  if (projectId) {
    for (const litter of unweanedLitters) {
      const enriched = enrichLitter(db, litter);
      if (enriched.projectId !== projectId) {
        const idx = unweanedLitters.indexOf(litter);
        if (idx > -1) unweanedLitters.splice(idx, 1);
      }
    }
  ...
  for (const rawLitter of unweanedLitters) {
    const litter = enrichLitter(db, rawLitter);
    const expectedWeanDate = rawLitter._expectedWeanDate || addDays(litter.birthDate, weaningDays);
    const finalWeanDate = expectedWeanDate < today ? today : expectedWeanDate;

    if (!dailyPressure[finalWeanDate]) {
      continue;
    }

    const dayEntry = dailyPressure[finalWeanDate];
    ...
    if (!dayEntry.byRoom[litterRoomId]) {
      dayEntry.byRoom[litterRoomId] = { maleCount: 0, femaleCount: 0, unknownSexCount: 0, totalCount: 0, litterIds: [] };
    }
    ...
    if (litterProjectId) {
      if (!dayEntry.byProject[litterProjectId]) {
        dayEntry.byProject[litterProjectId] = { maleCount: 0, femaleCount: 0, unknownSexCount: 0, totalCount: 0, litterIds: [] };
      }
    ...
    litterWeaningPlans.push({
      ...
      birthDate: litter.birthDate,
      expectedWeanDate: finalWeanDate,
      daysUntilWean: diffDays(today, finalWeanDate),
  ...
  const sortedLitters = [...litterWeaningPlans].sort((a, b) => a.daysUntilWean - b.daysUntilWean);
  ...
  for (const plan of sortedLitters) {
    ...
    const preferredProjectId = plan.projectId;
    ...
    for (const group of groups) {
      ...
      if (allocated < group.needAllocate) {
        ...
        riskItems.push({
          ...
          shortfall,
          reason: shortfall === group.needAllocate ? "无可用笼位" : "笼位容量不足",
  ...
  for (const dateStr of dateKeys) {
    ...
    for (const [rid, data] of Object.entries(entry.byRoom)) {
      if (!byRoomPressure[rid]) byRoomPressure[rid] = { maleCount: 0, femaleCount: 0, unknownSexCount: 0, totalCount: 0, litterIds: new Set(), peakDate: null, peakTotal: 0 };
    ...
    for (const [pid, data] of Object.entries(entry.byProject)) {
      if (!byProjectPressure[pid]) byProjectPressure[pid] = { maleCount: 0, femaleCount: 0, unknownSexCount: 0, totalCount: 0, litterIds: new Set(), peakDate: null, peakTotal: 0 };
  ...
  if (totalPending > 0) {
    ...
    if (riskRatio === 0) overallRisk = RISK_LEVEL.NONE;
    else if (riskRatio < 0.1) overallRisk = RISK_LEVEL.LOW;
    else if (riskRatio < 0.3) overallRisk = RISK_LEVEL.MEDIUM;
    else if (riskRatio < 0.6) overallRisk = RISK_LEVEL.HIGH;
