...
import { OCCUPANCY_STATUSES, ANIMAL_STATUS } from "./animalValidator.js";
import { PAIRING_STATUS } from "./breedingValidator.js";
import { ACTIVE_EVENT_STATUSES } from "./healthEventData.js";
...
function resolveProjectNameById(db, projectId) {
  return getProjectById(db, projectId)?.name || null;
}

function resolveFeedingProjectFromTarget(db, item) {
  if (item.targetType === "animal") {
    const animal = (db.animals || []).find(a => a.id === item.targetId);
    if (!animal) return { project: null, projectId: null };
    const projectId = animal.projectId || (animal.project ? resolveProjectIdByName(db, animal.project) : DEFAULT_PROJECT_ID);
    return {
      project: animal.project || resolveProjectNameById(db, projectId),
      projectId
    };
  }

  if (item.targetType === "cage") {
    const projectIds = [...new Set((db.animals || [])
      .filter(a => a.cageId === item.targetId && a.projectId)
      .map(a => a.projectId))];
    const projectId = projectIds.length === 1 ? projectIds[0] : DEFAULT_PROJECT_ID;
    return {
      project: resolveProjectNameById(db, projectId),
      projectId
    };
  }

  return {
    project: resolveProjectNameById(db, DEFAULT_PROJECT_ID),
    projectId: DEFAULT_PROJECT_ID
  };
}

function migrateFeedingFacilityFields(db, item) {
  let migrated = false;

  if (!item.roomId) {
    if (item.targetType === "cage") {
      const cage = (db.cages || []).find(c => c.id === item.targetId);
      item.roomId = cage?.roomId || DEFAULT_ROOM_ID;
    } else if (item.targetType === "animal") {
      const animal = (db.animals || []).find(a => a.id === item.targetId);
  ...
  if (!item.project || !item.projectId) {
    const resolved = resolveFeedingProjectFromTarget(db, item);
    if (!item.project && resolved.project) {
      item.project = resolved.project;
      migrated = true;
    }
    if (!item.projectId && resolved.projectId) {
      item.projectId = resolved.projectId;
      migrated = true;
    }
  }
...
export function migrateLegacyFacilityData(db) {
  ...
  if (db.feedingPlans) {
    for (const plan of db.feedingPlans) {
      if (migrateFeedingFacilityFields(db, plan)) migrated = true;
    }
  }
  if (db.feedingRecords) {
    for (const record of db.feedingRecords) {
      if (migrateFeedingFacilityFields(db, record)) migrated = true;
    }
  }
  if (db.healthEvents) {
...
export function resolveZoneIdByName(db, zoneName, roomId) {
  if (!zoneName) return null;
  ensureFacilityCollections(db);
  const zone = db.zones.find(z => {
...
export function getRoomPressureDashboard(db) {
  ensureFacilityCollections(db);
  const rooms = db.rooms || [];
  const allCages = db.cages || [];
  const allAnimals = db.animals || [];
  const allPairs = db.breedingPairs || [];
  const allEvents = db.healthEvents || [];

  const ACTIVE_PAIRING_STATUSES = [
    PAIRING_STATUS.PENDING,
    PAIRING_STATUS.MATED,
    PAIRING_STATUS.PREGNANT,
  ...
  for (const room of rooms) {
    ...
    const activeCages = roomCages.filter(c => c.status === "active");
    const disabledCages = roomCages.filter(c => c.status === "disabled");

    const totalCapacity = activeCages.reduce((s, c) => s + (c.capacity || 0), 0);

    const roomAnimals = allAnimals.filter(a => {
      const cage = allCages.find(c => c.id === a.cageId);
      return cage?.roomId === roomId && OCCUPANCY_STATUSES.includes(a.status);
    });

    const occupiedCount = roomAnimals.length;
    const occupancyRate = totalCapacity > 0 ? Number(((occupiedCount / totalCapacity) * 100).toFixed(2)) : 0;

    const quarantineCount = allAnimals.filter(a => {
      const cage = allCages.find(c => c.id === a.cageId);
      return cage?.roomId === roomId && QUARANTINE_STATUSES.includes(a.status);
    }).length;

    const breedingPairCount = allPairs.filter(p => {
      const cage = allCages.find(c => c.id === p.cageId);
      const resolvedRoomId = p.roomId || cage?.roomId;
      return resolvedRoomId === roomId && ACTIVE_PAIRING_STATUSES.includes(p.status);
    }).length;

    const pendingEventCount = allEvents.filter(e => {
      const resolvedRoomId = e.roomId;
      return resolvedRoomId === roomId && ACTIVE_EVENT_STATUSES.includes(e.status);
    }).length;
    ...
    dashboard[roomId] = {
      ...
      disabledCageCount: disabledCages.length,
      totalCapacity,
      occupiedCount,
      occupancyRate,
      quarantineAnimalCount: quarantineCount,
      breedingPairCount,
      pendingHealthEventCount: pendingEventCount
    };
  }

  const totalCapacityAll = Object.values(dashboard).reduce((s, r) => s + r.totalCapacity, 0);
  const totalOccupiedAll = Object.values(dashboard).reduce((s, r) => s + r.occupiedCount, 0);
  const overallOccupancyRate = totalCapacityAll > 0 ? Number(((totalOccupiedAll / totalCapacityAll) * 100).toFixed(2)) : 0;
  ...
  return {
    ...
    summary: {
      ...
      totalActiveCages: Object.values(dashboard).reduce((s, r) => s + r.activeCageCount, 0),
      totalDisabledCages: Object.values(dashboard).reduce((s, r) => s + r.disabledCageCount, 0),
      totalOccupied: totalOccupiedAll,
      totalCapacity: totalCapacityAll,
      overallOccupancyRate,
      totalQuarantineAnimals: Object.values(dashboard).reduce((s, r) => s + r.quarantineAnimalCount, 0),
      totalBreedingPairs: Object.values(dashboard).reduce((s, r) => s + r.breedingPairCount, 0),
      totalPendingHealthEvents: Object.values(dashboard).reduce((s, r) => s + r.pendingHealthEventCount, 0)
    }
  };
}
