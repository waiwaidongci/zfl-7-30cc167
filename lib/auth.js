...
export async function authenticate(req) {
  ...
  return {
    ...
    principal: {
      ...
      allowedRoomIds: keyRecord.allowedRoomIds,
      allowedProjectIds: keyRecord.allowedProjectIds,
      allowedZones: keyRecord.allowedZones
    }
  };
}
...
export class AuthorizationError extends Error {
  constructor(requiredRole, principal) {
    super(`角色 ${principal?.role || "未知"} 无权执行 ${requiredRole} 操作`);
    this.name = "AuthorizationError";
    this.requiredRole = requiredRole;
    this.principalRole = principal?.role || null;
  }
}

export function assertRole(principal, requiredRole) {
  if (!requireRole(principal, requiredRole)) {
    throw new AuthorizationError(requiredRole, principal);
  }
}
