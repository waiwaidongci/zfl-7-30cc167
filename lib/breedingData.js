...
export function calculateWeaningForecast(db, options = {}) {
  ...
  if (totalPending > 0) {
    ...
    if (riskRatio === 0 && overdueRatio === 0) overallRisk = RISK_LEVEL.NONE;
    else if (riskRatio < 0.1 && overdueRatio < 0.1) overallRisk = RISK_LEVEL.LOW;
    else if (riskRatio < 0.3 || overdueRatio < 0.3) overallRisk = RISK_LEVEL.MEDIUM;
    else if (riskRatio < 0.6 || overdueRatio < 0.6) overallRisk = RISK_LEVEL.HIGH;
