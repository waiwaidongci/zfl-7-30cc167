...
export function validateBatchImport(db, animalsInput, principal = null) {
  ...
  for (const { index, item } of fieldValidItems) {
    ...
    if (item.projectId) {
      ...
      const nameResolvedId = resolveProjectIdByName(db, item.project);
      const defaultProject = getProject(db, DEFAULT_PROJECT_ID);
      if (nameResolvedId === DEFAULT_PROJECT_ID && item.project !== defaultProject?.name) {
      ...
      resolvedProjectId = nameResolvedId;
      projectResolvedFrom = "projectName";
      autoResolved.push({ field: "projectId", from: "projectName", value: nameResolvedId, name: item.project });
    }
