...
function findProjectByName(db, projectName) {
  if (!projectName) return null;
  return (db.projects || []).find(p => p.name === projectName) || null;
}
...
export function validateBatchImport(db, animalsInput, principal = null) {
  ...
  const unresolvedProjectSet = new Set();
  ...
  for (const { index, item } of fieldValidItems) {
    ...
    if (!item.roomId) {
      autoResolved.push({ field: "roomId", from: "cage", value: cageRoomId, name: cageRoom?.name || cageRoomId });
    }
    if (!item.zoneId) {
      autoResolved.push({ field: "zoneId", from: "cage", value: cageZoneId, name: cageZone?.name || cageZoneId });
    }

    let resolvedProjectId = null;
    let projectResolvedFrom = null;
    ...
    if (item.projectId) {
      ...
      projectResolvedFrom = "projectId";
      const resolvedByNameId = findProjectByName(db, item.project)?.id;
      ...
      const nameProject = findProjectByName(db, item.project);
      if (!nameProject) {
        const key = `${index}:${item.project}`;
        if (!unresolvedProjectSet.has(key)) {
          unresolvedProjectSet.add(key);
          result.missingProjects.push({
            index,
            id: item.id || null,
            project: item.project,
            message: `项目名称 "${item.project}" 在系统中未找到匹配的项目，无法确定projectId`
          });
        }
        continue;
      }
      resolvedProjectId = nameProject.id;
      projectResolvedFrom = "projectName";
      autoResolved.push({ field: "projectId", from: "projectName", value: nameProject.id, name: item.project });
    }
    ...
    if (autoResolved.length > 0) {
      result.autoResolvedWarnings.push({
        index,
        id: item.id || null,
        cageId,
        fields: autoResolved
  ...
  for (const { index, item, cage, resolvedRoomId, resolvedZoneId, resolvedProjectId, autoResolved, cageRoomName, cageZoneName } of cageValidItems) {
    const id = item.id;
