...
import { resolveTargetOwnership } from "./targetOwnership.js";
...
const endpointPatterns = [
  ...
  { method: "GET", path: /^\/admin\/consistency\/check$/, action: ACTIONS.ADMIN },
  { method: "GET", path: /^\/admin\/consistency\/repair-preview$/, action: ACTIONS.ADMIN },
  { method: "GET", path: /^\/admin\/consistency\/summary$/, action: ACTIONS.ADMIN },
  { method: "GET", path: /^\/admin\/consistency\/categories$/, action: ACTIONS.ADMIN },
  { method: "GET", path: /^\/admin\/ledger\/rebuild-snapshot$/, action: ACTIONS.ADMIN },
  { method: "GET", path: /^\/admin\/ledger\/compare-snapshot$/, action: ACTIONS.ADMIN }
];
...
export function validateCrossRoomMove(db, principal, fromCageId, toCageId) {
  ...
  const fromOwnership = fromCageId ? resolveTargetOwnership(db, { targetType: "cage", targetId: fromCageId }) : null;
  const toOwnership = toCageId ? resolveTargetOwnership(db, { targetType: "cage", targetId: toCageId }) : null;

  const fromRoomId = fromOwnership?.roomId || DEFAULT_ROOM_ID;
  const toRoomId = toOwnership?.roomId || DEFAULT_ROOM_ID;
