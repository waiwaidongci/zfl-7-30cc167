...
const SYSTEM_EVENT_TYPES = [
  EVENT_TYPES.LEDGER_INITIALIZED,
  EVENT_TYPES.LEDGER_MIGRATED
];

const UNSUPPORTED_BUSINESS_EVENTS = [
  EVENT_TYPES.HEALTH_EVENT_CREATED,
  EVENT_TYPES.HEALTH_EVENT_CLOSED
];
...
export async function rebuildSnapshotFromLedger(options = {}) {
  ...
  const systemEvents = events.filter(e => SYSTEM_EVENT_TYPES.includes(e.eventType));
  const businessEvents = events.filter(e => !SYSTEM_EVENT_TYPES.includes(e.eventType));
  const replayableEvents = businessEvents.filter(e => REPLAY_EVENT_TYPES.includes(e.eventType));
  const otherSkippedEvents = businessEvents.filter(e => !REPLAY_EVENT_TYPES.includes(e.eventType));

  snapshot._meta.skippedEvents = systemEvents.length + otherSkippedEvents.length;

  for (const event of systemEvents) {
    snapshot._meta.skippedSystemEvents.push({
      eventId: event.id,
      eventType: event.eventType,
      reason: "system_event",
      description: getSkipReasonDescription("system_event", event.eventType)
    });
  }

  for (const event of otherSkippedEvents) {
    const reason = UNSUPPORTED_BUSINESS_EVENTS.includes(event.eventType)
      ? "unsupported_business_event"
      : "unknown_event_type";
    snapshot._meta.skippedOtherEvents.push({
      eventId: event.id,
      eventType: event.eventType,
      reason,
      description: getSkipReasonDescription(reason, event.eventType)
    });
  }
      ...
      switch (event.eventType) {
        ...
        default:
          snapshot._meta.unreplayableEvents.push({
            ...
            description: getUnreplayableReasonDescription("unsupported_event_type", event.eventType)
          });
      }
    } catch (error) {
      snapshot._meta.unreplayableEvents.push({
        ...
        description: getUnreplayableReasonDescription("processing_error", event.eventType, error.message),
        error: error.message
      });
    }
  }
  ...
  return {
    ...
    meta: {
      ...
      skippedSystemEvents: snapshot._meta.skippedSystemEvents,
      skippedOtherEvents: snapshot._meta.skippedOtherEvents,
      skippedSystemEventCount: snapshot._meta.skippedSystemEvents.length,
      skippedOtherEventCount: snapshot._meta.skippedOtherEvents.length,
      unreplayableEvents: snapshot._meta.unreplayableEvents,
      unreplayableEventCount: snapshot._meta.unreplayableEvents.length,
...
function getSkipReasonDescription(reason, eventType) {
  const descriptions = {
    "system_event": "系统事件，不参与业务快照回放",
    "unsupported_business_event": "业务事件类型暂不支持快照回放",
    "unknown_event_type": "未知事件类型，无法回放"
  };
  const base = descriptions[reason] || reason;
  return `${base} [${eventType}]`;
}

function getUnreplayableReasonDescription(reason, eventType, detail) {
  const descriptions = {
    "unsupported_event_type": "不支持回放的事件类型",
    "processing_error": "事件处理过程中发生错误",
    "missing_animal": "事件关联的动物不存在于账本中",
    "missing_required_field": "事件缺少必要字段",
    "entity_already_exists": "实体已存在，跳过创建"
  };
  const base = descriptions[reason] || reason;
  let desc = `${base} [${eventType}]`;
  if (detail) {
    desc += `: ${detail}`;
  }
  return desc;
...
function diffArrays(currentArray, rebuiltArray, idField, entityName, entityTypeLabel) {
  ...
  for (const [id, rebuiltItem] of rebuiltMap) {
    if (!currentMap.has(id)) {
      added.push({
        ...
        entity: entityName,
        item: rebuiltItem,
        reason: "only_in_ledger",
        reasonDescription: `仅存在于账本回放结果中，当前${entityTypeLabel || entityName}数据中不存在`
      });
    } else {
      const currentItem = currentMap.get(id);
      const fieldDiffs = diffObjects(currentItem, rebuiltItem, entityTypeLabel || entityName);
  ...
  for (const [id, currentItem] of currentMap) {
    if (!rebuiltMap.has(id)) {
      removed.push({
        ...
        entity: entityName,
        item: currentItem,
        reason: "missing_ledger_event",
        reasonDescription: `当前${entityTypeLabel || entityName}数据中存在，但账本中无对应事件记录，无法通过回放重建`
      });
    }
  }
...
function diffObjects(obj1, obj2, entityTypeLabel = "实体") {
  ...
  for (const key of allKeys) {
    ...
    if (!deepCompare(v1, v2)) {
      differences.push({
        ...
        reasonDescription: buildFieldDiffReason(entityTypeLabel, key, v1, v2)
      });
    }
  }
...
function buildFieldDiffReason(entityTypeLabel, field, currentValue, rebuiltValue) {
  const currentMissing = currentValue === undefined || currentValue === null;
  const rebuiltMissing = rebuiltValue === undefined || rebuiltValue === null;

  if (rebuiltMissing && !currentMissing) {
    return `当前${entityTypeLabel}的${field}字段存在，但账本回放结果缺少该字段或字段值，历史事件信息不足以重建`;
  }

  if (currentMissing && !rebuiltMissing) {
    return `账本回放结果包含${field}字段，但当前${entityTypeLabel}数据缺少该字段或字段值，当前快照无法直接解释该回放结果`;
  }

  return `当前${entityTypeLabel}的${field}字段与账本回放结果不一致，历史事件信息不足以确认当前值可由账本回放解释`;
}
...
export async function compareSnapshotWithCurrent(db, options = {}) {
  ...
  return {
    ...
    rebuiltMeta: {
      ...
      skippedSystemEvents: rebuiltResult.meta.skippedSystemEvents,
      skippedOtherEvents: rebuiltResult.meta.skippedOtherEvents,
      skippedSystemEventCount: rebuiltResult.meta.skippedSystemEventCount,
      skippedOtherEventCount: rebuiltResult.meta.skippedOtherEventCount,
      unreplayableEventCount: rebuiltResult.meta.unreplayableEventCount,
